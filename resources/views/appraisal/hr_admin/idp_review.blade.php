...
@section('content')
    ...
    <div class="container-fluid py-4 min-vh-100" style="background-color: #f8fafc;" x-data="idpHRReview()">
        <form @submit.prevent="submitReview">
            <div class="row mb-4 align-items-center">
                <div class="col-12 col-md-6">
                    <h1 class="h3 fw-bold text-dark mb-1">Review IDP Plan (HR Admin)</h1>
                    <p class="text-muted small mb-0">
                        Finalizing review for <span class="fw-bold text-primary">{{ $employee->name }}</span>
                        @if ($activeFY)
                ...
                <div class="col-12 col-md-6 text-md-end mt-3 mt-md-0 d-flex justify-content-md-end gap-2 align-items-center">
                    <a href="{{ route('idp.hr.list') }}" class="btn btn-outline-secondary px-4 shadow-sm">
                        <i class="fas fa-arrow-left me-2"></i> Back to List
                    </a>
                    <button type="submit" class="btn text-white px-4 shadow-sm" style="background-color: #1a6b3b;" :disabled="loading">
                        <template x-if="!loading">
                            <span><i class="fas fa-save me-2"></i> Save All Changes</span>
                        </template>
                        <template x-if="loading">
            ...
            <div class="card border-0 shadow-sm overflow-hidden mb-4">
                    ...
                    <div class="row align-items-center">
                        <div class="col-md-8">
                            <div class="d-flex align-items-center">
                                <div class="bg-primary p-3 rounded-circle me-3 text-white shadow-sm">
                                    <i class="fas fa-user-shield fa-lg"></i>
                                </div>
                                <div>
                                    <h5 class="mb-0 fw-bold text-dark">{{ $employee->name }}</h5>
                                    <p class="text-muted small mb-0">
                                        {{ $employee->employee_id }} | {{ $employee->designation }} | {{ $employee->department->name ?? 'N/A' }}
                                        <br>
                                        <span class="text-dark fw-medium">Line Manager:</span> {{ $employee->lineManager->name ?? 'N/A' }}
                                    </p>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-4 text-md-end">
                            <span class="text-muted small">Milestones:</span>
                            <span class="h4 fw-bold text-primary ms-1" x-text="rows.length"></span>
                        </div>
                    </div>
                </div>
                ...
                <div class="table-responsive">
                    <table class="table table-bordered align-middle mb-0 excel-style">
                        <thead style="background-color: #f8fbff; color: #1e40af;">
                            <tr>
                                <th style="width: 50px;" class="text-center small">SL</th>
                                <th style="width: 150px;" class="small">Skill area</th>
                                <th style="min-width: 300px;" class="small">Development Objective</th>
                            ...
                            <template x-for="(row, index) in rows" :key="index">
                                <tr>
                                    ...
                                    <td class="text-center">
                                        ...
                                        <span class="x-small" :class="row.is_approved ? 'text-success' : 'text-muted'" x-text="row.is_approved ? 'Approved' : 'Pending'"></span>
                                    </td>
                                </tr>
                            </template>
                        </tbody>
        ...
        function idpHRReview() {
            return {
                ...
                async submitReview() {
                    this.loading = true;
                    try {
                        const response = await fetch("{{ route('idp.hr.update', $idps->first()) }}", {
                            method: 'POST',
                            headers: {
                        ...
                        if (response.ok) {
                            window.location.href = "{{ route('idp.hr.list') }}";
                        } else {
