...
@section('content')
    <x-ui.datatable-card title="Edit Master Objective" subtitle="{{ $item->title_sentence_case }}" icon="fa-pen-to-square" body-class="p-4">
        <x-slot name="actions">
        ...
        <form method="POST" action="{{ route('departmental-objective-masters.update', $item) }}">
            @csrf
            @method('PUT')
